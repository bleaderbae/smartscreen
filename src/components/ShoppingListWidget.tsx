...
// Extracted and memoized to prevent re-renders when parent state (like input text) changes
const ShoppingListItem: React.FC<{
  item: ShoppingItem;
  onToggle: (id: string) => void;
  onRemove: (e: React.MouseEvent, id: string) => void;
}> = React.memo(({ item, onToggle, onRemove }) => {
...
const ShoppingListWidget: React.FC = () => {
  ...
  // Optimize: Memoize active item lookup to O(1) instead of O(N) inside the loop
  const activeItemTexts = useMemo(() => {
    const set = new Set<string>();
    items.forEach(i => {
      if (!i.completed) set.add(i.text.toLowerCase());
    });
    return set;
  }, [items]);
  ...
  const addItem = useCallback((text: string, iconName?: string, color?: string) => {
    setItems(prev => {
      // Avoid duplicates for quick add inside the setter to keep callback stable if desired,
      // though here we are just wrapping logic.
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
          {QUICK_ADD_ITEMS.map((preset) => {
            // Optimization: O(1) lookup using memoized Set
